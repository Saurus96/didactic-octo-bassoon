    ...
    <AnimatePresence>
      {isOpen ? (
        <>
          ...
          <motion.aside role="dialog" aria-modal="true" className="fixed inset-y-0 left-0 z-50 flex w-[84vw] max-w-[320px] flex-col rounded-r-[28px] p-4 glass glass-strong" initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
            ...
            <button type="button" onClick={onCreateChat} className="mb-4 w-full rounded-2xl border border-white/40 bg-[rgba(255,198,224,0.22)] px-4 py-3 text-left font-medium text-[#5c4856] shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] interactive">New Chat</button>

            <nav className="space-y-2">
              {navRows.map(({ id, label, icon: Icon }) => (
                <button key={id} onClick={() => { onChangeTab(id); onClose() }} className={`flex h-12 w-full items-center gap-3 rounded-2xl px-3 text-left text-sm interactive ${activeTab === id ? 'glass text-[#5a4a5d]' : 'text-[#6f6177] hover:bg-white/20'}`}>
            ...
            <div className="min-h-0 flex-1 space-y-2 overflow-y-auto rounded-2xl bg-white/8 p-2 text-sm text-[#6f6075] border border-white/20">
              ...
              {chats.map((chat) => (
                <div key={chat.id} className={`flex items-center gap-2 rounded-xl border p-1 ${activeChatId === chat.id ? 'bg-white/20 border-white/35' : 'bg-white/10 border-white/20'}`}>
